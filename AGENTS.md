...
- Em `Romaneios > Fechamento`, manter período inicial/final livre, listar todos os motoristas com romaneio no intervalo e permitir exportação individual por seleção. O Excel de cada motorista agrupa somente situações `Entregue` já gravadas por dia, com cidades, NFs únicas, frete diário e totais finais; situações manuais não entram nesse fechamento.
