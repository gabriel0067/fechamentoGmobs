...
- Agrupar a conferência dos romaneios por dia de emissão + motorista, ignorando o horário e reunindo todos os romaneios e rotas desse motorista no mesmo painel diário. Frete total, peso, entregas importadas e volumes devem considerar um único total por número de romaneio dentro do dia, usando o maior valor encontrado quando o mesmo romaneio vier repetido; só somar novamente quando houver dois números de romaneio diferentes no mesmo agrupamento. A quantidade exibida na ticagem e no romaneio completo deve vir dos documentos únicos extraídos da coluna `DOCUMENTOS`, somando NF manual e coletas; não usar a coluna `QTD. ENTREGAS` como total operacional quando ela divergir.
...
- A conferência diária deve permitir marcar `Entregue`, `Volta`, `Retorno` com motivo obrigatório, `Retido` ou `Não seguiu`, sempre com confirmação obrigatória nas situações diferentes de Entregue. Para registros cuja origem seja `LT` ou `RM`, eles permanecem fora do fechamento antes da conferência; somente depois de `Gravar conferência` passam a poder entrar no fechamento, convertidos em `ET` quando Entregues e em `OC` quando forem Volta, Retorno, Retido ou Não seguiu. A data operacional usada na liberação é o dia do romaneio. No fechamento do agregado, Entregue soma nota e produção, Retorno soma somente a nota, e Volta/Não seguiu não somam nota nem produção.
...
- Manter `Romaneios > Romaneio completo` como consulta separada por motorista, data, número do romaneio, cidade ou documento, exibindo documentos abertos e situações já gravadas.
