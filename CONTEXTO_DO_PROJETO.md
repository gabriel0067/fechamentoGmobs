...
Em 25/09/2026, a conferência de romaneios passou a manter a situação por romaneio, data e documento, preservando o histórico anterior e permitindo que uma nota finalizada como Volta ou Retorno apareça novamente em um romaneio posterior para receber uma nova situação. A consulta completa mostra também o operador que gravou cada ticagem. A baixa do Relatório de Retidos passou a localizar o documento do romaneio correto antes de atualizar sua liberação no fechamento, sem apagar o registro histórico. O fechamento agregado ganhou a opção `Fechar por: Placa do veículo`, que reúne a produção do mesmo carro mesmo quando houve motoristas diferentes. O frete importado do romaneio não é mais duplicado quando o relatório repete exatamente o mesmo total em várias linhas do mesmo romaneio; parcelas com valores diferentes continuam somadas. A regra possui teste automatizado.

Na mesma atualização, a digitação dos dados de nova coleta, do valor inicial de dedicado e dos campos editáveis da prévia de fechamento deixou de atualizar toda a aplicação a cada tecla; os valores são lidos ao confirmar ou ao sair do campo. A observação da conferência do romaneio já seguia esse modelo isolado. D&Y, TRD e Argius continuam exigindo bipagem e compartilham as leituras feitas em Capas com o Fechamento de Parceiros; as demais parceiras só usam esse filtro quando `Pagar somente notas bipadas` é ativado. A exclusão individual de dedicado permanece disponível dentro de `Editar detalhes`, com confirmação. Essas alterações não criam tabelas, não restauram backups e não removem dados existentes no Neon.

Em 23/09/2026, o Relatório de Retidos passou a mostrar `Gravar baixas` como botão flutuante no canto inferior direito assim que ao menos um documento é selecionado. O botão acompanha a quantidade selecionada e desaparece depois da gravação ou quando a seleção é desfeita, evitando percorrer uma lista grande até o final. A alteração é somente de interface e não modifica o esquema nem os dados existentes no Neon.

Em 23/09/2026, a proteção de concorrência foi ajustada porque a primeira implementação trocava toda a interface pela tela de banco indisponível quando duas pessoas gravavam o mesmo conjunto. Agora o conflito é reconciliado automaticamente em três vias: usa a versão originalmente carregada para identificar somente o que mudou localmente e remotamente, preserva inclusões independentes e respeita exclusões locais, grava o resultado combinado sobre a versão mais recente e atualiza a tela com esse resultado. Falhas reais de rede durante uma gravação não desmontam mais a tela nem escondem o trabalho: aparece apenas uma faixa de sincronização pendente com opção de tentar novamente. A tela bloqueante permanece somente quando o carregamento inicial do banco falha, pois nesse caso não existe uma base segura para trabalhar. A reconciliação roda apenas quando há conflito, portanto não acrescenta processamento ao uso normal nem desfaz as otimizações de navegação e pesquisa.

Em 22/09/2026, foi identificada uma disputa entre usuários simultâneos no armazenamento Neon: a versão anterior enviava o conjunto inteiro de cada estado e aceitava a última gravação mesmo quando outro navegador já havia atualizado o mesmo conjunto. A nova versão passa a exigir a versão que o navegador leu antes de confirmar a gravação no banco; se ela divergir, o banco mantém os dados existentes e o navegador mostra erro, sem substituir a alteração de outra pessoa. Nessa tela de erro há opção de baixar uma cópia do trabalho pendente antes de recarregar ou tentar novamente. Gravações consecutivas do mesmo navegador são enfileiradas por conjunto. A verificação periódica das seis versões passa a usar uma consulta conjunta, reduzindo chamadas ao servidor. Isso protege contra perda por sobrescrita e reduz parte da carga simultânea, mas não transforma os conjuntos grandes em registros individuais: conflitos entre dois operadores editando o mesmo conjunto ainda exigem reconciliação manual, e os tempos de gravação com vários usuários autenticados precisam ser medidos antes de afirmar que a lentidão acabou. Nenhum esquema nem dado existente é alterado por esta etapa.

Em 21/09/2026, foi preparada, ainda sem publicação, uma correção adicional para Capas e Dedicados. A duplicidade de Capas passa a comparar CT-e/chave antes de NF: NFs iguais com CT-es diferentes são permitidas; o mesmo CT-e permanece bloqueado. Uma leitura deixa de incluir repetidamente linhas de referência equivalentes, e a seleção da parceira reduz ambiguidades de NF. A geração da capa registra o item antes de carregar o módulo de PDF e informa separadamente se o download falhar; a confirmação definitiva continua sendo o estado de salvamento no banco. Os campos textuais e observação de Dedicados deixam de recalcular a tela a cada tecla e há exclusão individual com confirmação. A marca d'água do nome da parceira nos PDFs de embarque, capas e coleta ficou mais visível. A compilação e o teste de backup/migração passaram; o lint mantém erros preexistentes. Falta validar o fluxo com dados reais, gerar e conferir backup atual da produção e só então publicar. Os backups existentes e o site antigo não foram alterados.

Em 22/09/2026, testes na versão local confirmaram que uma capa manual permanece no histórico após recarregar, NF igual com CT-es diferentes gera dois itens, e o mesmo CT-e é bloqueado. Um PDF de embarque foi renderizado e a marca d'água ficou legível. Uma restauração local de cópia com 14.503 entradas e 695 romaneios revelou que o `localStorage` não comportava todo o conjunto de romaneios: eles apareciam na sessão, mas sumiam ao recarregar. O estado local de romaneios foi movido para o IndexedDB existente, com migração conservadora do formato anterior e aviso explícito em caso de falha; uma restauração sintética foi confirmada após recarregar. O site hospedado usa Neon, não esse armazenamento local. Antes da publicação, foi criado no Neon um snapshot manual da branch `production` em 22/09/2026 07:32:28 UTC, sem expiração; os seis conjuntos (`billed`, `closing`, `maex`, `romaneios`, `scans`, `tde`) estavam presentes em consulta somente de leitura. O site antigo e os arquivos de backup anteriores permanecem intactos.

Em 21/09/2026, após o operador confirmar que a versão anterior ainda travava na abertura, navegação e romaneios, foi iniciada uma correção mais ampla na branch de migração. A montagem dos documentos de cada romaneio passou a usar índices por chave em vez de procurar repetidamente na lista acumulada; o vínculo com o relatório reutiliza resultados ordenados para a mesma referência e data. Os índices de busca dos romaneios são reutilizados entre pesquisas enquanto os dados não mudarem. A lista de Retidos deixou de percorrer todos os documentos para cada registro, e os agrupamentos de faltas e não identificados deixaram de copiar arrays a cada inclusão. O fechamento dos motoristas e a comparação da Pajuçara são calculados somente quando as respectivas telas estão abertas. A serialização, compactação e leitura dos estados da nuvem agora podem ocorrer em um Web Worker, com fallback para o processamento anterior em navegadores incompatíveis, para manter a interface responsiva durante operações grandes. As bibliotecas pesadas de planilha/PDF passaram a ser carregadas somente ao importar ou exportar; as funções pequenas de normalização/cálculo permanecem no carregamento inicial. A compilação e os testes de migração/período/normalização passaram. O commit `aa7cd2c` foi enviado ao GitHub e a prévia `https://fechamento-gmobs-3ck5rp2qt-gmobs.vercel.app/` ficou Ready, sem erros de console na tela de login. A produção ainda não foi promovida: falta validar com dados reais autenticados os tempos de abertura, mudança de menu, pesquisa, digitação e gravação. O armazenamento de conjuntos inteiros no Neon permanece uma limitação arquitetural para a duração da gravação, embora o trabalho pesado de codificação saia da linha principal da interface. Nenhum dado, backup, esquema ou site antigo é alterado por esta correção.

Em 20/09/2026, a etapa seguinte de desempenho (`10bb896`) foi promovida manualmente à produção da Vercel no deployment `FejUUXQUFq5p2SfB9zmMKF8KfVTK`, com status Ready. As buscas de conferência e consulta de romaneios, Retidos, capas, coletas, dedicados e adicionais passaram a exigir confirmação por botão `Pesquisar` ou Enter; digitar sozinho não filtra as listas nem reconstrói a tela. A conferência de romaneios mantém a ação separada `Localizar / marcar Entregue` para bipagem. A lista inicial passou de 40 para 15 romaneios por vez, e os índices de texto das duas consultas de romaneio só são criados quando existe uma pesquisa aplicada. O `content-visibility: auto` foi retirado dos cartões de romaneio por poder deixar cartões visíveis em branco durante travamentos de pintura. A prévia de fechamento monta a tabela nota a nota somente quando expandida e mostra 100 linhas por vez, sem limitar os totais ou a exportação; as listas de bipagem mostram 50 itens por vez. O botão `Gravar conferência` agora fica fixo no canto inferior direito somente enquanto um romaneio aberto tiver situações preparadas; ao fechar o painel ou gravar, desaparece. Essas medidas reduzem trabalho desnecessário de interface, mas ainda falta medir com usuário autenticado o carregamento inicial e a troca entre todas as guias; o estado inteiro do romaneio continua sendo sincronizado após alterações.
...
- A escolha inicial de Capas oferece `Relatório` no mesmo padrão visual dos controles do sistema.
- Na importação geral, uma linha nova com a mesma chave/CT-e de uma nota anterior continua fora das cobranças duplicadas, mas sua linha completa é guardada como referência do romaneio. Assim, reentregas, voltas e retornos que saem em um novo romaneio podem ser associados ao relatório e localizados pela NF, CT-e ou chave. As referências são preferidas pela proximidade da data de emissão do romaneio. Linhas que já haviam sido descartadas por versões anteriores precisam ser reimportadas para voltar à base de consulta; a reimportação não apaga os registros já salvos.
